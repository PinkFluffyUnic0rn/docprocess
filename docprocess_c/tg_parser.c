...
int tg_rel(int ni)
{
	...
	nni = tg_nodeadd(ni, TG_N_NEXTTO, &t);
	tg_nextto(nni);
...
int tg_assign(int ni)
{
	...
	tni = tg_nodeadd(ni, TG_N_TERNARY, &t);
	tg_ternary(tni);
	
	while (tg_peektoken(&t) == TG_T_ASSIGN) {
		tg_gettokentype(&t, TG_T_ASSIGN);
...
int tg_expr(int ni)
{
	...
	ani = tg_nodeadd(ni, TG_N_ASSIGN, &t);
	tg_assign(ani);
